...
const state = {
  ...
  editorMessages: [
    { role: "user", text: "请为「深圳市东星制冷机电有限公司」生成完整企业官网，行业方向：制冷机电。参考「工业自动化」的风格和栏目，但可以自由调整页面结构、布局和交互，以页面美观和资料准确为优先。页面内容使用知识库中的企业信息、产品资料、图片、案例、资讯、资质和联系方式。" },
    ...
    pages: [
      {
        ...
        blocks: [
          ...
          },
          {
            ...
            media: "图 + 文",
            layout: "card-grid",
            content: "把知识库中的重点产品前置展示，帮助客户快速判断适配范围。",
            items: [
      ...
      },
      {
        id: "products",
        name: "产品中心",
        purpose: "把知识库中的产品拆成清晰分类，方便客户快速判断适配范围。",
        blocks: [
...
function renderTopNav() {
  ...
  const compactIds = activeNavItem && !["knowledge", "site", "geo"].includes(activeNavItem.id)
    ? ["site", "geo", activeNavItem.id]
    : ["knowledge", "site", "geo"];
...
function renderAuthed() {
  ...
  return `
    <header class="app-header">
      <button class="brand-chip" data-section="knowledge" type="button" aria-label="返回 GEO Studio 首页">
        <span>G</span><strong>GEO Studio</strong>
      </button>
      ${renderTopNav()}
      <button class="notice-btn" type="button">暂无新的留资信息</button>
...
function renderKnowledgePage() {
  ...
  return `
    ${pageHead("企业知识库", "统一沉淀企业事实、产品、案例与资质，后续可同时服务 360智见 GEO 项目和官网生成项目。", `<div class="head-actions"><span class="done-pill">字段 42 项</span><span class="doc-count">· 来源 21</span><button class="primary slim" data-section="site" type="button">创建官网</button></div>`)}
...
function renderKnowledgeIntake() {
  return `
    <section class="source-intake" aria-label="知识库信息导入">
    ...
    <section class="site-center">
      ...
      <div class="prompt-card">
        ...
        <div class="prompt-actions">
          <div><button class="ghost slim" data-section="knowledge" type="button">企业资料 (19)</button><button class="ghost slim selected" data-template-open type="button">工业自动化（默认）</button></div>
...
function renderEditor() {
  return `
    <header class="editor-header">
      ...
      <div><button class="ghost slim" type="button">预览</button><button class="primary slim" type="button">发布上线</button><button class="notice-btn" type="button">暂无新的留资信息</button><span class="phone-mask">${maskPhone(state.phone)}</span><button class="logout-icon" id="logout" type="button" aria-label="退出登录">${icon("i-log-out")}</button></div>
    </header>
...
function renderPagesPage() {
  ...
  return `
    ${pageHead(
      "页面模板",
      "先确认频道页面、页面结构和知识库生成内容；未修改时默认使用当前结构进入生成。",
      `<div class="head-actions"><span class="done-pill">默认已确认</span></div>`
    )}
    ...
    <section class="page-template-workbench">
      ...
      <section class="structure-preview">
        ...
        <div class="structure-next">
          <div>
            <b>默认确认规则</b>
            <span>用户不调整频道或模块时，将直接以当前结构生成带风格官网，并对标题、段落、FAQ、Schema 做 GEO 改写。</span>
          </div>
        </div>
      </section>
    </section>
...
function renderPublishPage() {
  return `${pageHead("发布与域名", "域名、SSL、发布检查、版本回滚")}
    <section class="stats-grid">${stat("域名状态","已解析","DNS 生效于 3 分钟前","good")}${stat("SSL 证书","有效","到期还有 86 天","good")}${stat("发布版本","v1.8","可随时回滚")}${stat("上线检查","11 / 14","3 项需要处理","warn")}</section>
    <section class="grid-2"><div class="panel"><h2>域名绑定</h2><div class="domain-card"><b>www.zhizao-demo.cn</b><span>主域名</span><small>CNAME -> publish.360zhiwang.com</small></div><div class="dns-box">CNAME www publish.360zhiwang.com<br />TXT @ 360zw-verify=82ca19</div></div><div class="panel checklist"><h2>发布前检查</h2>${["移动端布局检查通过","表单接收邮箱已配置","sitemap.xml 已生成","新闻资讯页面内容为空","FAQ 页面未发布","2 个按钮链接未配置"].map((v,i)=>`<label><input type="checkbox" ${i<3?"checked":""}/> ${v}</label>`).join("")}</div></section>`;
}

function renderAnalyticsPage() {
  return `${pageHead("数据分析", "访问、转化、GEO 曝光与内容缺口")}
    <section class="stats-grid">${stat("本周访问","12,480","环比 +18%","good")}${stat("询盘转化","3.6%","表单 42 条")}${stat("AI 搜索曝光","326","品牌相关问答","good")}${stat("内容缺口","14","集中在价格/交付")}</section>
    <section class="grid-2"><div class="panel chart-panel"><h2>访问与转化趋势</h2><div class="chart">${[38,45,32,54,62,58,73,69,82,78,92,88].map(v=>`<i style="height:${v}%"></i>`).join("")}</div></div><div class="panel"><h2>GEO 曝光问题</h2>${["工业视觉检测系统哪家公司好？","自动化产线改造周期多久？","设备数据采集网关支持哪些 PLC？"].map(v=>`<div class="queue"><b>${v}</b><span>建议补充案例、FAQ 或品牌型号</span></div>`).join("")}</div></section>`;
}
...
function bindEvents() {
  ...
  $$("[data-quick]").forEach(btn => btn.addEventListener("click", () => {
    ...
    state.editorMessages.push({ role: "ai", text: "已收到优化需求，正在根据知识库、页面结构和 GEO Prompt 更新官网预览。" });
    render();
  }));
